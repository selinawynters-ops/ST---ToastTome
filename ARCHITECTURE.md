...
├── index.js               # Main extension logic and panel views
...
- **DOM structure**:
  ...
  └── .tt-footer                 (counts + Concealments/Transcribe/Erase/Seal)
  ```
- **Event listeners**:
  - Search input → `renderEntries()` on every keystroke
  - Filter pills → sets `activeFilter`, re-renders
  - Concealments → opens or closes the suppression recovery view
  - Allow Again / Restore All → removes one or all stored block patterns
...
- **Keyboard**: ESC returns from the Concealments manager before closing the popup

#### `openSuppressionManager()` / `closeSuppressionManager()` / `renderSuppressionManager()`
- **What**: Switches the existing panel between toast history and suppression recovery.
- **Behavior**: Lists `settings.hideList`, restores individual patterns, clears all patterns, and renders an empty state.
- **CSS**: `.tt-panel--managing` swaps `.tt-body`/`.tt-toolbar` for `.tt-suppression-manager`.
...
| 2. Popup | ~400 | `.tt-panel*`, `.tt-titlebar*`, `.tt-toolbar*`, `.tt-body*`, `.tt-entry*`, `.tt-suppression*`, `.tt-footer*` | History popup and concealments manager |
...
| `.tt-suppression-manager` | Suppression recovery view inside the panel |
| `.tt-suppression-item` | Stored suppressed pattern with an Allow Again control |
